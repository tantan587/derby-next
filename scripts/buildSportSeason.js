...
sport_ids.forEach(sport_id => {
    ...
    let to_add = []
    for(let i=1; i<4; i++){
        let api_pull = sport_id===107 ? 383 : i === 1 ? new_year : i===2 ? String(new_year).concat('PRE') : String(new_year).concat('POST')
        data.push({
            sport_season_id: season_id, 
            sport_id: sport_id,
            year: new_year,
            season_type: i,
            start_pull_date: '2019-08-01',
            end_pull_date: '2020-10-01',
