...
const updateBoxScoreJSON = async (exitProcess, whichPull) => {
  
  let today_games
  let thePullArr = whichPull.split(',')
  switch(thePullArr[0])
  {
  case 'active':
  {
    today_games = await pullActiveGamesFromKnex()
    break
  }
  case 'near':
  {
    today_games = await pullNearGamesFromKnex()
    break
  }
  case 'previous':
  {
    today_games = await previousGamesFromKnexBySport(thePullArr[1])
    break
  }
  case 'specific':
  {
    today_games = await pullGamesForSpecificDay(thePullArr[1])
    break
  }
  }
...
const pullActiveGamesFromKnex = async () => {
  let now = new Date()
...
const pullNearGamesFromKnex = async () => {
  let now = new Date()
  let dayCount = getDayCount(now)
  let today_games = await knex('sports.schedule')
    .whereIn('day_count', [dayCount-1, dayCount, dayCount + 1])
    .whereIn('sport_id', [101,103,104,105,106])
  return today_games
}

const pullGamesForSpecificDay = async (dayCount) => {
  let today_games = await knex('sports.schedule')
